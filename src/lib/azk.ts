...
const triangularInverse = (n: number) => Math.floor((Math.sqrt(1 + 8 * n + 1) - 1) / 2);

export const createBoardLayout = (maxTileCount: number, tileSize: number): BoardLayout => {
    ...
    const toPixelCoords = (q: number, r: number) => ({ x: tileSize * (sqrt3 * q + sqrt3 * r / 2), y: tileSize * 1.5 * r });
    ...
    return {tileWidth, tileHeight, boardMinX, boardMinY, boardWidth, boardHeight, tiles};
}
 ...
 */
export const calcPolygonVertices = (n: number, size: number, rotation: number = 0): {x: number, y: number}[] => {
